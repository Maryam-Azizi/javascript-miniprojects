...
function submit(){
	...
	if (i == -1){
		document.querySelector('#result').innerHTML= "Search not found";
			...
			var output = boldSearch(sentence,search);
    
			document.querySelector('#result').innerHTML= output;
		}
}
function boldSearch(sentence,search){
		var i = sentence.indexOf(search);
		if (i== -1)return sentence;
		var output = sentence.substring(0,i)+'<strong>'+search+ '</strong>'+boldSearch(sentence.substring(i+search.length),search);
		return output;
}
function addEvent(){
